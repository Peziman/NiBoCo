...
uint16_t get_pot_duty(uint8_t a_input) //for display
{
...
{ 
  pumpspeed = constrain(pumpspeed, no_speed, max_speed);  // Begrenzt den Bereich der PWM Ausgabe auf 34 bis 220.
...
int16_t get_PWM (uint16_t clt, uint16_t low_point, uint16_t high_point, bool run)
{
  ...
  else
  {
    ...
    if (d_cnt > d_loops)
    {
      if (t_error - t_error_old < 0)
      {
        D = (t_error - t_error_old) * kd;
